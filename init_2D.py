...
## Generator points from the 2D honeycomb:
...
#plotter_2D(np.concatenate(edge_assignments_cel(p,q),axis=0))
...
def polygon_assignments(p,q, state=None):
    ...
    e1=edge_assignments_cell_1(p,q,state)
    e2=edge_assignments_cell_2(p,q,state)

    polys=[Polygon(e1),Polygon(e2)]
    return polys
